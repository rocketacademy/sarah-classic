...
Let's run code outside the browser console. We'll create a basic program with a SWE101 starter template.
...
Try changing `distanceInKilometers` to see new conversions. To let the user input distances, assign the value of `input` to `distanceInKilometers`.
