...
Install linter packages to auto-format your JavaScript code in `script.js`. **Note that you will need to run `npm install` every time you clone the starter code in Coding Basics.**
...
{% hint style="info" %}
You may see a popup asking you to use ESLint in VSCode. Please select "Allow Everywhere" in the popup. 

If you don't get auto-formatting on save after running `npm i`, check if you see a red ESLint status icon in the bottom right corner of VSCode. If you do, follow instructions [here](../course-logistics/tooling-pro-tips.md#eslint-not-running-after-first-install) to enable ESLint in VSCode.
{% endhint %}
