...
We will control random number input to test winning conditions by removing randomness. We will fix the "random" number by changing our `rollDice` function to always return a specific value.
...
Note that the return keyword also ends the execution of the function. `rollDice` never runs beyond line 2.
