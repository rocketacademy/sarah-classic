...
We've been able to build relatively complex programs using the control flow syntax of functions and conditionals with global state. Great work so far!

In this module we will learn about a new data type, or "data structure" to be precise: dynamic-length data "**arrays"**. We'll then learn how to manipulate arrays using the control structure called "**loops**". Just like each concept we learned previously, the mechanics will be relatively simple, but the complexity of the apps we can build will increase greatly.
