...
If we put a loop inside a loop we can represent 2 dimensions of output. Note we use `<br>` to create new rows. `<br>` is an HTML tag, and we use it because our program interprets `myOutputValue` as HTML.
