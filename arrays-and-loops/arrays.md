...
const shells = [null, null, null];

const randomBallLocation = getRandomInteger(3);

shells[randomBallLocation] = 'win!';

const main = function (input) {
  let myOutputValue = 'sorry wrong guess';
...
const main = function (input) {
  ...
  let myOutputValue = `you lose. Your guesses: ${guesses}`;
...
while (index < letterLength) {
  const currentLoopLetter = letters[index];
...
const main = function (input) {
  ...
  const myOutputValue = `All your names: ${names}`;
