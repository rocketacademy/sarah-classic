...
2. Create a variable for our function: `var convertKmToMiles`. **Note that function variable names start with a verb by convention, to distinguish them from variables storing other data types. With few exceptions, function names should almost always start with a verb.**
...
Use the `return` keyword to define the output value of the function. Note that the output value of a function may not be the output value of the overall program, because functions can call other functions. The output value of the overall SWE101 starter code program is the return value of the `main` function.
