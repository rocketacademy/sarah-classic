...
For consistency and to avoid bugs, ESLint recommends that we always define functions above where they are used, even if technically our programs may still work in certain cases where functions are defined below where they are used. Rocket Academy follows this recommendation.
