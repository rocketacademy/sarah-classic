...
2. Logic sequences that we wish to encapsulate in a _"sub-routine"_. We do this with our `main` function. `main` contains everything we want to happen when the user clicks Submit. It is common for sub-routine functions to rely on several other __ helper functions.
